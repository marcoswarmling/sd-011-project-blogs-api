...
const putPostMiddleware = (req, res, next) => {
const { title, content, categoryIds } = req.body;
  if (!title) {
    return res.status(status.badRequest).json({ message: postMessages.titleRequired });
  }
  ...
  if (categoryIds) {
    return res.status(status.badRequest).json({ message: postMessages.noEditCategory });
  }
  next();
};

module.exports = { postMiddleware, putPostMiddleware };
